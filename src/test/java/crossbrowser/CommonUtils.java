...
public class CommonUtils extends CrossBrowserTest {															//Method extends from the crossbrowsertest class
	...
	//This method is used to login to the C1 platform
		public static void loginTest() throws InterruptedException, Exception {
...
//			  userName.sendKeys(username);
			...
			userName.sendKeys("IOCQAT1@mailsac.com");
...
//			  passwordele.sendKeys(password);
			passwordele.sendKeys("CUPIOCQA#T1", Keys.ENTER);
			
			 
		/*	
			wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id='gigya-login-form']/div[2]/div[1]/input")));
		...
		//	wait.until(ExpectedConditions.elementToBeClickable(By.xpath("(//div[@class='gigya-layout-row']//div[@class='gigya-composite-control gigya-composite-control-submit nemo-login-btn']//input[@class='gigya-input-submit'])[4]")));
			...
			//WebElement loginBtn = driver.findElement(By.xpath("(//div[@class='gigya-layout-row']//div[@class='gigya-composite-control gigya-composite-control-submit nemo-login-btn']//input[@class='gigya-input-submit'])[4]"));
			
			
			WebElement loginBtn = driver.findElement(By.xpath("//*[@id='gigya-login-form']/div[2]/div[1]/input"));
			loginBtn.click();
...
//			boolean extrasButtonFound = driver.findElement(By.xpath("//*[@class='extras_button full_screen']")).isDisplayed();
		...
		public static void launchinglotestnew() 
		{
		    ...
		    try {
		        ...
		        if (s2.isDisplayed()) {
		            ...
		            return; // Exit the method since we've already handled the case where s2 is displayed
		        }
		    } 
		    ...
		    {
		        // s2 is not available, continue execution
		    	System.out.println("Screen 2 is not available");
		    }
		...
		//	String lt = driver.findElement(By.xpath("//*[@id='activity-header']/ul/li[1]/span")).getText();
			...
			else if(s1.isDisplayed() && s2.isDisplayed())
			{
...
//				zoomInPage();
		 ...
		 * System.out.println("multiple choice single"); frameex(); webEleList =
...
/*		
		if (!activityFound) {
		    ...
		    System.out.println("size::web elements::" + webEleList.size());
		    Actions action = new Actions(driver);
		    if (!webEleList.isEmpty()) {
		        Integer sourceIndex = activityCountMap.getOrDefault(activityType + ".source", 0);
		        for (int i = sourceIndex; i < tokens.size(); i++) {
		            WebElement source = tokens.get(i);
		            if (source.isDisplayed()) {
		                activityCountMap.put(activityType + ".source", sourceIndex + 1);
		                Integer destIndex = activityCountMap.getOrDefault(activityType + ".dest", 0);
		                if (destIndex < webEleList.size()) {
		                    WebElement dest = webEleList.get(destIndex);
		                    if (dest.isDisplayed()) {
		                        activityCountMap.put(activityType + ".dest", destIndex + 1);
		                        wait.until(ExpectedConditions.elementToBeClickable(dest));
...
//		Drag and Drop
		...
		//	{
			//if (webEleList != null && webEleList.size() > 0) 
		    if (!webEleList.isEmpty() && !tokens.isEmpty())
			{
				Integer sourceIndex = activityCountMap.get(activityType + ".source");
				for (int i = sourceIndex; i < tokens.size(); i++) {
					WebElement source = tokens.get(i);
		...
		if(!activityFound ) {
//			Drag and Drop source click and releasing it in the destination
				System.out.println("DragNDrop clicking the source and the target elements ");
				activityType = "target";
				xpath = props.getProperty("act." + activityType);
				dndXpath = xpath.split(":");
				System.out.println(xpath);
				System.out.println(dndXpath[0] + " -------- " + dndXpath[1]);
				frameex();
				tokens = driver.findElements(By.xpath(dndXpath[1]));
				webEleList = driver.findElements(By.xpath(dndXpath[0]));
				System.out.println("size::web elements::" + webEleList.size());
				
				
				
				System.out.println("Enter the D&D category manually and enter an integer and click enter");
		...
		if (!activityFound) {
		    ...
		    if (webEleList != null && webEleList.size() > 0) {
		        for (int i = 0; i < webEleList.size(); i++) {
		            WebElement target = webEleList.get(i);
		                ...
		                for (int j = 0; j < tokens.size(); j++) {
		                    ...
		                    if (source.isDisplayed()) {
		                        try {
		                            ...
		                            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", source);
		                            source.click();
		                        }
		                        try {
		    ...
		    if (activityFound) {
		        Thread.sleep(8000); // Consider using explicit waits instead of Thread.sleep()
		        nextScreen();
		    } else {
		        frameclose();
		    }
		...
		if (!activityFound) {
		    ...
		    int destEleSize = webEleList.size();
		    if (destEleSize <= 0) {
		        webEleList = driver.findElements(By.xpath(dndXpath[3]));
		        destEleSize = webEleList.size();
		    }
		    System.out.println("size::web elements::" + destEleSize);
		    System.out.println("tokens size::" + tokens.size());
		    if (webEleList != null && destEleSize > 0) {
		        ...
		        Thread.sleep(3000);
		        Integer sourceIndex = activityCountMap.get(activityType + ".source");
		        int totalSources = tokens.size();
		        int sourceProcessed = 0; // Counter to track the number of processed source elements
		        while (sourceProcessed < totalSources) {
		            for (int i = 0; i < webEleList.size(); i++) {
		                WebElement ele = webEleList.get(i);
		                if (ele.isDisplayed()) {
		                    activityCountMap.put(activityType + ".source", i + 1);
		                    ...
		                    System.out.println("size::token::" + t);
		                    boolean actperf = false;
		                    int j = sourceProcessed; // Start iterating from the last processed source
		                    while (j < t) {
		                        WebElement source = tokens.get(j);
		                        if (source.isDisplayed()) {
		                            try {
		                            ...
		                            System.out.println("source index :: " + j);
		                            actperf = true;
		                            sourceProcessed++; // Increment the processed source count
		                        } else {
		                            j++;
		                            System.out.println("source hidden:: " + j);
		                        }
...
//			zoomInPage();
	...
	case "Multiple Choice (multiple answers)_RDP":
		...
		//Integer startIndex = activityCountMap.get(activityType);
			...
			// int endIndex = Math.min(startIndex + 9, webEleList.size()); // Process at most 3 elements
			...
			for (int i = 0; i < webEleList.size(); i++) {
				WebElement ele = webEleList.get(i);
				if (ele.isDisplayed()) {
					System.out.println("Elements visible");
					activityCountMap.put(activityType, i + 1);
					activityFound = true;
					((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", ele);
					((JavascriptExecutor) driver).executeScript("arguments[0].click();", ele);
	...
	case "Presentation_RDP":
		...
		try {
			...
			//WebElement path2 =	driver.findElement(By.xpath("//a[contains(text(),'NEXT ACTIVITY ')]"));
		...
		} catch (Exception ex) 
		 {
	        ...
	        try {
	            ...
	            try {
	                WebElement path2 = driver.findElement(By.xpath("//a[contains(text(),'NEXT ACTIVITY ')]"));
	                if (path2.isDisplayed()) {
	                    System.out.println("Next activity option is visible.");
	                    path2.click();
	                }
	            } catch (Exception e2) {
	                System.out.println("Fallback options not found. Cannot proceed.");
	            }
	        }
	    }
	    break;
...
//					ele.click();
	...
	case "Flashcards_RDP":
		...
		activityType = "flipcards";
		frameex();
		xpath = props.getProperty("act." + activityType);
		WebElement fc = driver.findElement(By.xpath(xpath));
		//fc.click();
		
		try
		{
				fc.click();
				System.out.println("Flipcard is present");
			} 
		...
		catch(Exception e)
		{
			...
			System.out.println(e);
			System.out.println("Flipcard is not present");
